...
ACMD(do_examine) {
    ...
    if (tmp_object) {
        if ((GET_OBJ_TYPE(tmp_object) == ITEM_DRINKCON) || (GET_OBJ_TYPE(tmp_object) == ITEM_FOUNTAIN) || (GET_OBJ_TYPE(tmp_object) == ITEM_CONTAINER)) {
...
ACMD(do_score) {
    ...
    send_to_char(
        ch,
        "Your armor class is %d/10, and your alignment is %d.\r\n",
    ...
    if (GET_LEVEL(ch) < LVL_IMMORT) {
        send_to_char(
            ch,
            "You need %d exp to reach your next level.\r\n",
    ...
    send_to_char(
        ch,
        "You have been playing for %d day%s and %d hour%s.\r\n",
    ...
    if (!*arg) {
        ...
        for (i = character_list; i; i = i->next) {
            if (CAN_SEE(ch, i) && IN_ROOM(i) != NOWHERE && isname(arg, i->player.name)) {
                ...
                send_to_char(ch, "M%3d. %-25s - [%5d] %s\r\n", ++num, GET_NAME(i), GET_ROOM_VNUM(IN_ROOM(i)), world[IN_ROOM(i)].name);
            }
        }
...
ACMD(do_levels) {
    ...
    for (i = 1; i < LVL_IMMORT; i++) {
        nlen = snprintf(buf + len, sizeof(buf) - len, "[%2d] %8d-%-8d : ", i, level_exp(GET_CLASS(ch), i), level_exp(GET_CLASS(ch), i + 1) - 1);
...
ACMD(do_commands) {
    ...
    send_to_char(
        ch,
        "The following %s%s are available to %s:\r\n",
        wizhelp ? "privileged " : "",
