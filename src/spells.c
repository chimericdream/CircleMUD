...
ASPELL(spell_identify) {
    ...
    if (obj) {
        ...
        if (!IS_NPC(victim)) {
            send_to_char(
                ch, "%s is %d years, %d months, %d days and %d hours old.\r\n",
        ...
        send_to_char(
            ch,
            "AC: %d, Hitroll: %d, Damroll: %d\r\n",
            compute_armor_class(victim),
            GET_HITROLL(victim),
            GET_DAMROLL(victim)
        );
        send_to_char(
            ch, "Str: %d/%d, Int: %d, Wis: %d, Dex: %d, Con: %d, Cha: %d\r\n",
        ...
        switch (GET_OBJ_TYPE(obj)) {
            ...
            case ITEM_FOOD:
                if (GET_OBJ_VAL(obj, 3)) {
                    ...
                    act(
                        "You sense that $p is safe for consumption.", FALSE, ch, obj, 0,
                        TO_CHAR
                    );
                }
