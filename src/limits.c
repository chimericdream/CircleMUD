...
void gain_exp(struct char_data *ch, int gain) {
    ...
    if (gain > 0) {
        ...
        while (GET_LEVEL(ch) < LVL_IMMORT - immort_level_ok && GET_EXP(ch) >= level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1)) {
            GET_LEVEL(ch) += 1;
            num_levels++;
            advance_level(ch);
            is_altered = TRUE;
        ...
        if (is_altered) {
            mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s advanced %d level%s to level %d.", GET_NAME(ch), num_levels, num_levels == 1 ? "" : "s", GET_LEVEL(ch));
