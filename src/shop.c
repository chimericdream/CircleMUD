...
int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr) {
    ...
    if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) || (IS_CLERIC(ch) && NOTRADE_CLERIC(shop_nr)) || (IS_THIEF(ch) && NOTRADE_THIEF(shop_nr)) || (IS_WARRIOR(ch) && NOTRADE_WARRIOR(shop_nr))) {
...
void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr) {
    ...
    while (obj && (GET_GOLD(ch) >= buy_price(obj, shop_nr, keeper, ch) || IS_GOD(ch)) && IS_CARRYING_N(ch) < CAN_CARRY_N(ch) && bought < buynum && IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch)) {
        int charged;
...
/*
   The function below is a slight hack!  To make sure that duplicate items are
...
char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr, struct char_data *keeper, struct char_data *ch) {
    ...
    switch (GET_OBJ_TYPE(obj)) {
        ...
        case ITEM_WAND:
        case ITEM_STAFF:
            snprintf(itemname, sizeof(itemname), "%s%s", obj->short_description, GET_OBJ_VAL(obj, 2) < GET_OBJ_VAL(obj, 1) ? " (partially used)" : "");
