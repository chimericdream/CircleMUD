    ...
    while (!feof(fl)) {
        ...
        if (!feof(fl)) {
            if (real_object(object.item_number) != NOTHING) {
                ...
                #else
                send_to_char(ch, " [%5d] (%5dau) %-20s\r\n", object.item_number, GET_OBJ_RENT(obj), obj->short_description);
                #endif
                extract_obj(obj);
            }
        }
...
int Crash_load(struct char_data *ch) {
    ...
    mudlog(NRM, MAX(GET_INVIS_LEV(ch), LVL_GOD), TRUE, "%s (level %d) has %d object%s (max %d).", GET_NAME(ch), GET_LEVEL(ch), num_objs, num_objs != 1 ? "s" : "", max_obj_save);
...
void Crash_rent_deadline(struct char_data *ch, struct char_data *recep, long cost) {
    ...
    snprintf(
            buf, sizeof(buf), "$n tells you, 'You can rent for %ld day%s with the gold you have\r\n"
...
int Crash_offer_rent(struct char_data *ch, struct char_data *recep, int display, int factor) {
    ...
    if (display) {
        ...
        snprintf(
            buf,
            sizeof(buf),
            "$n tells you, 'For a total of %ld coins%s.'",
            totalcost,
...
int gen_receptionist(struct char_data *ch, struct char_data *recep, int cmd, char *arg, int mode) {
    ...
    if (free_rent) {
        act("$n tells you, 'Rent is free here.  Just quit, and your objects will be saved!'", FALSE, recep, 0, ch, TO_VICT);
    ...
    if (CMD_IS("rent")) {
        ...
        if (mode == RENT_FACTOR) {
            ...
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s has rented (%d/day, %d tot.)", GET_NAME(ch), cost, GET_GOLD(ch) + GET_BANK_GOLD(ch));
        }
        else { /* cryo */
            act(
