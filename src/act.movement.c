...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check) {
    ...
    need_movement = (movement_loss[SECT(IN_ROOM(ch))] + movement_loss[SECT(EXIT(ch, dir)->to_room)]) / 2;
    ...
    if (ROOM_FLAGGED(EXIT(ch, dir)->to_room, ROOM_TUNNEL) && num_pc_in_room(&(world[EXIT(ch, dir)->to_room])) >= tunnel_size) {
        if (tunnel_size > 1) {
...
#define LOCK_DOOR(room, obj, door) ((obj) ? (SET_BIT(GET_OBJ_VAL(obj, 1), CONT_LOCKED)) : (SET_BIT(EXITN(room, door)->exit_info, EX_LOCKED)))
...
#define TOGGLE_LOCK(room, obj, door) ((obj) ? (TOGGLE_BIT(GET_OBJ_VAL(obj, 1), CONT_LOCKED)) : (TOGGLE_BIT(EXITN(room, door)->exit_info, EX_LOCKED)))
...
#define DOOR_IS_OPENABLE(ch, obj, door) ((obj) ? ((GET_OBJ_TYPE(obj) == ITEM_CONTAINER) && OBJVAL_FLAGGED(obj, CONT_CLOSEABLE)) : (EXIT_FLAGGED(EXIT(ch, door), EX_ISDOOR)))
        ...
        for (door = 0; door < NUM_OF_DIRS; door++) {
            if (EXIT(ch, door)) {
                if (EXIT(ch, door)->to_room != NOWHERE) {
                    if (!EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED) && ROOM_FLAGGED(EXIT(ch, door)->to_room, ROOM_INDOORS)) {
                        perform_move(ch, door, 1);
