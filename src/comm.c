...
int parse_ip(const char *addr, struct in_addr *inaddr) {
    log("SYSERR: warning: you're trying to set DFLT_IP but your system has no " "functions to parse IP addresses (how bizarre!)");
...
int perform_subst(struct descriptor_data *t, char *orig, char *subst) {
    ...
    /* terminate "first" at the position of the '^' and make 'second' point to the beginning of the second string */
    *(second++) = '\0';
    ...
    /* now, if there's anything left in the original after the string to replaced, copy that too. */
